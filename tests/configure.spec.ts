...
test.group('Configure', (group) => {
  ...
  test('publish provider and env variables', async ({ assert, fs }) => {
    ...
    await assert.fileContains('adonisrc.ts', '@adonisjs/lock/lock_provider')

    await assert.fileContains('config/lock.ts', [
      `  default: env.get('LOCK_STORE'),`,
      `redis: stores.redis`,
      `memory: stores.memory`,
    ])
    await assert.fileContains('.env', 'LOCK_STORE')
  ...
  test('throw error when select store is invalid', async ({ fs }) => {
    ...
    command.assertLog(
      'Invalid lock store "foo". Supported stores are: database, redis, and dynamodb'
    )
  })
